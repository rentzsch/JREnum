/**	
   ...
   https://github.com/rentzsch/JREnum	
	Bitwise implementation attemp by Alex Gray, http://github.com/mralexgray
...
#define JREnum(ENUM_TYPENAME, ENUM_CONSTANTS...)											\
	typedef enum {	ENUM_CONSTANTS } ENUM_TYPENAME;											\
...
#define JREnumDefine(ENUM_TYPENAME) _JREnum_GenerateImplementation(ENUM_TYPENAME)
...
#define _JREnum_GenerateImplementation(ENUM_TYPENAME)  																\
	...
	NSDictionary* ENUM_TYPENAME##ByValue() {																				\
		...
		for (NSUInteger i = 0; i < constants.count; i += 2)															\
			[result setObject:(NSString*)constants[i] forKey:(NSNumber*)constants[i+1]];						\
		return result;																												\
	}																																	\
